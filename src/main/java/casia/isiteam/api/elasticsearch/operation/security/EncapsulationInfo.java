...
public class EncapsulationInfo extends EsDbUtil {
    protected String url(String driverName){
        ...
        return Validator.check(ipPort) ?HTTP+ ipPort[0] : null;
    }
                ...
                if( !Validator.check(s.getKeywordsCombines()) ){
                    ...
                    //keyword格式
                        ...
                        if( s.getField().contains(DOT2) ){
                            int index = s.getField().lastIndexOf(DOT2);
                            String path = s.getField().substring(0,index);
                            matchjson.put(NESTED, o(o(PATH,path),QUERY, o(BOOL,o(SHOULD,a(o( s.getQueriesLevel().getLevel(), s.getQueriesLevel().getLevel()== QueriesLevel.All.getLevel() ? o() :
                            o(s.getField(),s.getKeyWord())))))));
                        }else{
