            ...
            if( aggsLevel.isPresent()){
                ...
                //Term
                 if( key.startsWith(QUESTION+ AggsLevel.Term.getLevel()+QUESTION ) ){
                    ...
                    AggsInfo info = new AggsInfo(AggsLevel.Term.getName(),key.replaceAll(AGGS_FIELD,NONE),os.containsKey(DOC_COUNT)?os.getLong(DOC_COUNT):-1,-1, (os.containsKey(SUM_OTHER_DOC_COUNT)?os.getLong(SUM_OTHER_DOC_COUNT):-1));
                    if( os.containsKey(BUCKETS) ){
                        for(Object s : os.getJSONArray(BUCKETS)){
                            JSONObject object = JSONObject.parseObject(s.toString());
                            AggsInfo infoc =  new AggsInfo(AggsLevel.Term.getName(),object.getString(KEY),object.containsKey(KEY_AS_STRING)?object.getString(KEY_AS_STRING):null,
                                    object.getLong(DOC_COUNT),-1, (object.containsKey(SUM_OTHER_DOC_COUNT)?object.getLong(SUM_OTHER_DOC_COUNT):-1) );
                        ...
                        parseAggesResult( info.getChildren(), os);
                    }
                    aggsInfos.add(info);
                }
                //Operation
                    ...
                    AggsInfo info = new AggsInfo(aggsList.stream().filter(s->key.startsWith(QUESTION+  s.getLevel()+QUESTION)).findFirst().get().getName()
                            ,key.replaceAll(AGGS_FIELD,NONE),os.containsKey(DOC_COUNT)?os.getLong(DOC_COUNT):-1,Validator.check(os.get(VALUE))?os.getFloat(VALUE):-1);
                ...
                // date
                     ...
                     JSONObject os = json.getJSONObject(key);
                     AggsInfo info = new AggsInfo( AggsLevel.Date.getName(),key.replaceAll(AGGS_FIELD,NONE),os.containsKey(DOC_COUNT)?os.getLong(DOC_COUNT):-1);
                     if( os.containsKey(BUCKETS) ){
                         for(Object s : os.getJSONArray(BUCKETS)){
                             JSONObject object = JSONObject.parseObject(s.toString());
                ...
                //Price
                    ...
                    JSONObject os = json.getJSONObject(key);
                    AggsInfo info = new AggsInfo(AggsLevel.Price.getName(),key.replaceAll(AGGS_FIELD,NONE),os.containsKey(DOC_COUNT)?os.getLong(DOC_COUNT):-1,-1);
                    if( os.containsKey(BUCKETS) ){
                 ...
                 //IPRange
                     ...
                     JSONObject os = json.getJSONObject(key);
                     AggsInfo info = new AggsInfo(AggsLevel.IPRange.getName(),key.replaceAll(AGGS_FIELD,NONE),os.containsKey(DOC_COUNT)?os.getLong(DOC_COUNT):-1,-1);
                     if( os.containsKey(BUCKETS) ){
                         os.getJSONObject(BUCKETS).forEach((n,m)->{
                             JSONObject newchildern = JSONObject.parseObject(String.valueOf(m));
